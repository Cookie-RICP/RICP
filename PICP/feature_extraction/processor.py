...
class CookieFeatureProcessor:
    ...
    def __init__(self, feature_parameter_source: str, skip_cmp_cookies: bool) -> None:
        ...
        # tracks the current features in sparse representation 稀疏矩阵
    ...
    def retrieve_label_weights(self, num_labels: int) -> List[float]:
        ...
        logger.info(f"Computed Weights: {inverse_ratio}, {num_total}, {self._labels}")
    ...
    def extract_features_with_labels(self, input_data: Dict[str, Dict[str, Any]]) -> None:
        """
        用于训练数据特征提取. 输入字典中应包含标签.
        过滤不需要的标签和不需要的名称.
        执行时间测量以分析特征提取性能.
        :param input_data: 要转换的Cookie训练数据，带有标签.
        """
        ...
        for entry_name, entry_values in tqdm(input_data.items()):
            ...
            # This includes name, domain, path and first-party domain
            for feature in self.feature_mapping["per_cookie_features"]:
                if feature["enabled"]:  # 都满足
                    # 过滤缺失值，检查该对象是否拥有该属性
                    assert hasattr(self, feature["function"]), f"Defined per-cookie function not found: {feature['function']}"
                    # 添加特征功能
                    if feature["function"] not in timings_per_function:
                        timings_per_function[feature["function"]] = list()  # 添加提取过程计时器
                    # 得到该特征的功能
                    function = getattr(self, feature["function"])

                    t_start = time.perf_counter_ns()
                    function(entry_values, **feature["args"])  # 转为稀疏矩阵
            ...
            self._reset_col()
            self._increment_row()
            result = [self._col_indices[0]]
            for i in range(1, len(self._col_indices)):
                if self._col_indices[i] > result[-1]:
                    result.append(self._col_indices[i])
                else:
                    result = [self._col_indices[i]]
            print()
            print(result)
            print(self._data_entries[-len(result):])
            print()
    ...
    def feature_top_names(self, cookie_features: Dict[str, Any]) -> None:
        ...
        if cookie_name in self._top_name_lookup:  # 如果该样本cookie的name在顶级500个名称之中
            rank = self._top_name_lookup[cookie_name]  # 给该样本指定的排序值
    ...
    def feature_top_domains(self, cookie_features: Dict[str, Any]) -> None:
        ...
        cookie_domain: str = url_to_uniform_domain(cookie_features["domain"])  # 将域名统一格式
    ...
    def feature_gestalt_mean_and_stddev(self, cookie_features: Dict[str, Any]) -> None:
        ...
        if len(diffs) > 0:
            self._insert_sparse_entry(mean(diffs), col_offset=0)  # 计算平均值
        else:
