...
def get_fixed_params() -> Dict[str, Union[None, float, int, str]]:
    ...
    return {
        ...
        'random_strength': 1,
        'max_depth': 12,  # up to max of 16  12  CPU —最大16
        'grow_policy': "Lossguide",  # ,"SymmetricTree" Lossguide
        ...
        'max_leaves': 48,  # only used for lossguide  53 48
...
def save_catboost_model(bst: catb.CatBoostClassifier) -> None:
    """
    Save the given booster model.
    :param bst: Computed booster.
    """
    ...
    file_path = os.path.join(model_path, f"catbmodel_{now_str}.cbm")
    bst.save_model(file_path)
    logger.info(f"Model dumped to '{file_path}'")
...
def output_validation_statistics(bst: catb.CatBoostClassifier, ntree: int, dtest: csr_matrix, y_test: List[int], class_names: List[str]) -> None:
    ...
    maxprob_labels = np.argmax(predicted_probabilities, axis=1)
    incorrect_predictions = np.where(labels_as_nparray != maxprob_labels)
    dense_matrix = dtest.toarray()  # 转化为矩阵
    for i in incorrect_predictions:
        print(f"样本 {i}：")
        print(f"真实值：{labels_as_nparray[i]}")
        print(f"预测值：{maxprob_labels[i]}")
        print(f"预测概率：{predicted_probabilities[i]}")

    for i in incorrect_predictions[0]:
        for j in range(1689):
            if dense_matrix[i][j] != 0:
                print(i, j, dense_matrix[i][j], end=";")
        print()

# 2 1051 1.0 2 1552 1.0 2 1555 2.0 2 1562 1.0 2 1563 -1.0 2 1565 -1.0 2 1566 11.0 2 1568 19.0 2 1570 1.8585552118703144 2 1572 -1.0 2 1573 -1.0 2 1574 -1.0 2 1575 -1.0 2 1576 1.0 2 1577 -1.0 2 1578 31535999.0 2 1579 -1.0 2 1580 -1.0 2 1581 -1.0 2 1582 -1.0 2 1583 -1.0 2 1584 -1.0 2 1585 1.0 2 1586 -1.0 2 1587 11.0 2 1588 19.0 2 1589 -8.0 2 1590 1.8585552118703144 2 1591 -1.0 2 1592 -1.0 2 1593 -1.0 2 1594 -1.0 2 1595 -1.0 2 1596 -1.0 2 1597 -1.0 2 1598 -1.0 2 1599 -1.0 2 1600 -1.0 2 1601 -1.0 2 1602 -1.0 2 1653 -1.0 2 1654 -1.0 2 1655 -1.0 2 1656 -1.0 2 1657 -1.0 2 1658 -1.0 2 1659 -1.0 2 1660 -1.0 2 1661 -1.0 2 1662 -1.0 2 1663 -1.0 2 1664 -1.0 2 1665 -1.0 2 1666 -1.0 2 1667 -1.0 2 1668 -1.0 2 1669 1.0 2 1670 -1.0 2 1671 -1.0 2 1672 -1.0 2 1673 -1.0 2 1674 -1.0 2 1675 -1.0 2 1676 -1.0 2 1677 -1.0 2 1678 -1.0 2 1679 -1.0 2 1680 -1.0 2 1681 -1.0 2 1682 -1.0 2 1683 -1.0 2 1684 -1.0 2 1685 -1.0 2 1687
    print(dense_matrix)
...
def split_train(X: csr_matrix, y: List[int], weights: Optional[List[float]], train_fraction: float = 0.8) -> None:
    ...
    bst.set_params(**params)
    bst.fit(X_train, y_train, use_best_model=True, eval_set=evalset, verbose_eval=True, plot=True)
    ...
    # 将损失函数值保存到文件
    with open('loss_values3.pkl', 'wb') as file:
    ...
    plt.show()
    # 输出特征的重要性
    # fea_ = bst.feature_importances_
    # fea_name = bst.feature_names_
    # print(fea_name)
    # print(fea_)
    # 获取特征重要性和特征名称
    fea_importance = bst.get_feature_importance()
    fea_names = bst.feature_names_

    # 保存为CSV文件
    np.savetxt('feature_importance.csv', fea_importance, delimiter=',')
    # 根据特征重要性排序
    sorted_indices = fea_importance.argsort()[::-1]
    sorted_importance = fea_importance[sorted_indices]
    # Produce s。，tatistics on validation set (confusion matrix, accuracy, etc.)
    output_validation_statistics(bst, limit, X_test, y_test, class_names)
    # Save the model to the computed models subfolder
    save_catboost_model(bst)
...
def crossvalidate_train(X: csr_matrix, y: Optional[List[int]],
    ...
    cv_results_path = os.path.join(eval_path, f"cv_results_1.pkl")
